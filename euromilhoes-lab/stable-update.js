(()=>{
'use strict';
const APP_VERSION='4.2.1';let lastCheck=0,checking=false;
...
async function loadEuroDreams(){try{await loadScript('ed-store.js');await loadScript('ed-base.js');await loadScript('ed-ui.js')}catch(err){console.warn('Módulo EuroDreams indisponível:',err)}}
...
document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible'){checkForAppUpdate(false);loadEuroDreams()}});window.addEventListener('online',()=>checkForAppUpdate(true));window.addEventListener('pageshow',()=>{checkForAppUpdate(false);loadEuroDreams()});showVersion();loadEuroDreams();checkForAppUpdate(true);window.EUROLAB_APP_VERSION=APP_VERSION;
})();